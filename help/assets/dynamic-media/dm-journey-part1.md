---
title: Percorso Dynamic Medie, Parte I
...
Questo percorso descrive le nozioni di base di Dynamic Medie, il suo funzionamento, il suo potenziale vantaggio e il valore che apporta al tuo lavoro e ai tuoi clienti.
...
* L’accesso a Dynamic Medie su Experience Manager è utile, ma non obbligatorio

**_Cosa puoi aspettarti di imparare_**

_Parte I_

* Cos&#39;è Dynamic Medie e come può aiutarti?
* Casi d’uso per Dynamic Medie
* Flusso di una risorsa attraverso il sistema Dynamic Medie
...
**_Pubblico_**
I tipi di pubblico più adatti ai lettori di questo percorso sono i seguenti, che non hanno mai utilizzato Dynamic Medie su Experience Manager:
...
## Cos&#39;è Dynamic Medie e come può aiutarti? {#dm-journey-a}

Dynamic Medie ti consente di offrire risorse visive avanzate di marketing e merchandising su richiesta. Consente inoltre di creare e distribuire esperienze di visualizzazione interattive, tra cui zoom, 360 gradi in rotazione e video. Le risorse vengono scalate in modo dinamico per l’utilizzo su siti web, mobili e social network. Utilizzando una serie di risorse di origine principali, come immagini, video e 3D, Dynamic Medie genera e distribuisce in tempo reale più varianti di questi rich content attraverso la rete CDN (Content Delivery Network) globale, scalabile e ottimizzata per le prestazioni.

Dynamic Medie incorpora i flussi di lavoro della soluzione Adobe Experience Manager Assets per la gestione delle risorse digitali per semplificare e semplificare il processo di gestione delle campagne digitali.

### Un file con infinite possibilità

Uno dei punti principali da capire su Dynamic Medie è il concetto di _Un file di risorsa principale con infinite possibilità_.
...
![Adobe Dynamic Medie fornisce la stessa immagine primaria a supporti diversi in formati e dimensioni diversi](/help/assets/dynamic-media/assets/dm-oneasset-multioutput.png)
_Adobe Dynamic Medie garantisce un&#39;esperienza coerente e di qualità su qualsiasi schermo, indipendentemente dalle dimensioni o dalla larghezza di banda._
...
Dynamic Medie è già in grado di ottimizzare le risorse e garantire che ogni risorsa venga caricata rapidamente sui sistemi mobili e desktop tramite la rete CDN. Per realizzare questo obiettivo, in Dynamic Medie vengono utilizzati predefiniti per immagini per definire la qualità dell&#39;immagine. Inoltre, definiscono il tipo di immagine che stai inviando, la sua nitidezza e altri elementi per varie parti delle esperienze o delle pagine.

Tuttavia, per aggiungere ancora più valore a Dynamic Medie oltre ai predefiniti per immagini, esiste _Imaging avanzato_.
...
_La tecnologia Smart Imaging ottimizza automaticamente il formato e le dimensioni dei file di un&#39;immagine in base alla capacità del browser e alla velocità di rete del cliente._

L’imaging avanzato non è attivato per impostazione predefinita perché richiede uno sforzo coordinato tra l’utente e il supporto tecnico Adobe Dynamic Medie. Inoltre, l’abilitazione di Smart Imaging richiede la cancellazione completa della cache CDN, che viene quindi riempita con il tempo. Se sei interessato a utilizzare Smart Imaging, puoi lavorare con Adobe per attivarlo inviando un ticket di supporto tecnico. Il supporto tecnico fornisce quindi un parametro URL che consente di provare, in anticipo, l’imaging intelligente. Puoi provarlo su qualsiasi pagina web o immagine in modo da vedere le prestazioni che ottieni e i risparmi. È quindi possibile attivare la tecnologia Smart Imaging per l&#39;intero sito.
...
L’utilizzo di set video adattivi offre una riproduzione fluida e di alta qualità per i clienti che riproducono video Dynamic Medie su più schermi e dispositivi. Riduce davvero la complessità del video.

## Casi d’uso per Dynamic Medie {#dm-journey-b}

Di seguito sono riportati i problemi comuni relativi ai casi d’uso e le soluzioni che Dynamic Medie può aiutarti a gestire per favorire il coinvolgimento, la fedeltà, la conversione e l’aumento del ROI dei clienti.
...
_Spessore della pagina del contenuto di una pagina web tipica._

Il restante 10% è HTML, codice CSS e tag specifici. Desideri ottimizzare il peso del 90% di quella pagina e Dynamic Medie ti aiuta con questo sforzo. Prima hai letto del concetto di _un file di risorsa principale con infinite possibilità_. Questo approccio è importante per ridurre il peso complessivo delle pagine. La possibilità di prendere una risorsa principale e utilizzarla in una pagina di dettaglio del prodotto, in una pagina di miniature, nel carrello e nella griglia di ricerca ti consente di risparmiare tempo. Inoltre, assicura la coerenza tra le diverse esperienze.

![Approccio basato sul file principale](/help/assets/dynamic-media/assets/dm-onefile.png)
_L’orologio è un file di risorse principale, ma con più rappresentazioni, non copie, create al volo._

Esaminiamo più da vicino i problemi che Dynamic Medie sta risolvendo con un file e alcune delle soluzioni a questo approccio.

| **Problema** | **Soluzione Dynamic Medie** |
|---|---|
...
| I video si interrompono o vengono riprodotti in bassa qualità a causa della larghezza di banda disponibile dell&#39;utente. | Distribuisci il video attraverso un lettore HTML che rileva automaticamente la larghezza di banda disponibile e adatta la qualità per garantire un&#39;alta fedeltà e una riproduzione fluida. |
...
| I membri del team scaricano e creano versioni locali. | I membri del team utilizzano un singolo file principale per creare _e_ fornire tutte le versioni necessarie su schermi e dispositivi di varie dimensioni. |
...
| **Problema** | **Soluzione Dynamic Medie** |
|---|---|
| Disegnare, misurare e tagliare manualmente immagini o video per evidenziare il punto focale e visualizzarli in modo appropriato su tutti i dispositivi e le dimensioni dello schermo richiede molto tempo e lavoro. | Utilizza il ritaglio avanzato in Dynamic Medie, una funzionalità di intelligenza artificiale di Adobe Sensei, per rilevare automaticamente il punto focale in qualsiasi immagine o video e ritagliarlo per mantenerlo. |
...
## Flusso di una risorsa attraverso il sistema Dynamic Medie {#dm-journey-c}

Di seguito è riportato un flusso di lavoro tipico per Dynamic Medie.

![Flusso di lavoro di Dynamic Medie](/help/assets/dynamic-media/assets/dm-workflow.png)
_Flusso di una risorsa attraverso il sistema Dynamic Medie._
...
Al termine della creazione, la risorsa viene inserita nella soluzione di authoring caricandola in Dynamic Medie. In Dynamic Medie, assicurati di disporre dei predefiniti immagine e dei visualizzatori giusti allineati per le varie pagine web del sito.

Infine, ottimizzate tutti i contenuti e li pubblicate sui server Dynamic Medie in modo che siano disponibili per il web, la stampa, la posta elettronica, i desktop e i dispositivi mobili.

### Caricamento di risorse in Dynamic Medie

Al termine della creazione, la risorsa principale viene caricata in Dynamic Medie. Il tipo di file caricato, il formato e le dimensioni del file sono attributi importanti per Dynamic Medie. È al momento del caricamento che desideri assicurarti di ottenere il valore massimo dal supporto di un file.

Ad esempio, l&#39;immagine dell&#39;orologio qui sotto è di 4560 x 3020 pixel. E anche se non si può mai usare un&#39;immagine di quelle dimensioni, è comunque possibile caricarla. Più grande è l&#39;immagine, migliore è la qualità offerta da Dynamic Medie, fino alla rappresentazione in miniatura. Ricorda: puoi facilmente _diminuzione_ risoluzione di un&#39;immagine esistente. Ma se provi a _aumentare_ risoluzione di un&#39;immagine, il risultato sarà probabilmente insoddisfacente.

![Formati consigliati per il caricamento in Dynamic Medie](/help/assets/dynamic-media/assets/dm-upload-formats.png)
_Considerazioni per il caricamento delle risorse._

L’Adobe consiglia di caricare le risorse in un formato senza perdite. In genere, è meglio evitare JPEG perché quando distribuisci JPEG o continui a salvare JPEG, la qualità delle immagini si perde nel tempo. Si desidera iniziare con le immagini con la massima risoluzione in un formato senza perdita di dati. Tale formato è in genere un file TIFF o PNG.

Per quanto riguarda lo spazio colore, quando si pensa a un canale digitale o a una visualizzazione web, in genere si pensa al RGB (rosso, verde, blu).

La maggior parte non penserebbe mai a consegnare qualcosa in CMYK o perché si potrebbe anche volere consegnare in CMYK. Il motivo è che lo spazio colore viene utilizzato più spesso per la consegna di articoli stampati. Ma Dynamic Medie è in grado di fornire in entrambi gli spazi di colore.

Ci sono molti clienti che ancora stampano, come i club all&#39;ingrosso di magazzino. E ci sono negozi di alimentari che stampano spesso volantini su base settimanale. Tali clienti richiedono che le loro immagini siano in entrambi gli spazi di colore. Tradizionalmente, questo richiedeva due immagini diverse: una in RGB e una in CMYK. Tuttavia, puoi caricare le risorse CMYK direttamente in Dynamic Medie, e far sì che Dynamic Medie distribuisca automaticamente le risorse RGB tramite un predefinito immagine o un profilo colore. Non è necessario creare più versioni di un file, mantenendo così il concetto di _un file di risorsa principale con infinite possibilità_.

<!-- **The Value of Renditioning??? or Demo portion** -->

### Pubblicare e visualizzare in anteprima le risorse

Dopo aver caricato le risorse in Dynamic Medie, è buona norma _pubblicare_ selezionandole e facendo clic su **[!UICONTROL Pubblica]** o **[!UICONTROL Pubblicazione rapida]** in Dynamic Medie. La pubblicazione delle risorse è necessaria se intendi utilizzarle in qualsiasi esperienza. Una volta pubblicate, le risorse possono essere incluse in una pagina web utilizzando un URL generato da Dynamic Medie e copiato oppure incorporando codice nella pagina.

Oltre a pubblicare manualmente le risorse, puoi configurare Dynamic Medie in modo da pubblicare immediatamente le risorse al momento del caricamento, senza alcun intervento da parte dell’utente.

Dopo il caricamento, esistono diversi modi per visualizzare in anteprima le rappresentazioni di una risorsa in Dynamic Medie. L’anteprima delle rappresentazioni può essere utile per avere un’idea di ciò che vede un cliente. Un metodo comune per l’anteprima consiste nel selezionare una risorsa e visualizzarne le rappresentazioni selezionando una _predefinito immagine_ come illustrato di seguito.
...
_Anteprima del rendering di una risorsa in base al predefinito immagine &quot;Large&quot; selezionato. È stato fatto clic sul pulsante URL. Il percorso URL risultante contiene il nome del predefinito immagine &quot;Large&quot; (Grande) e può essere utilizzato in una pagina web._

L&#39;URL qui sopra è live! [Prova](http://s7d1.scene7.com/is/image/jpearldemo/AdobeStock_28563982?$Large$){target="_blank"}.

Un altro metodo per visualizzare in anteprima una risorsa consiste nel selezionare la risorsa immagine e quindi selezionare una _Visualizzatori_ come illustrato di seguito.
...
_Visualizzazione in anteprima di una risorsa in base al predefinito visualizzatore &quot;ZoomVertical_light&quot; selezionato. Il puntatore del mouse (`+`) è stato spostato sull&#39;orologio per ingrandire. Osserva i pulsanti URL e Incorpora._
...
* [Utilizzare Dynamic Medie in Experience Manager](/help/assets/dynamic-media/dynamic-media.md)
* [Informazioni sulla tecnologia Smart Imaging](/help/assets/dynamic-media/imaging-faq.md)
...
* [Come distribuire risorse Dynamic Medie](/help/assets/dynamic-media/delivering-dynamic-media-assets.md)
