...
#### Invia per HTTP {#pushing-through-http}
...
#### Estrarre dal file system locale {#pulling-from-the-local-filesystem}

L’importatore [di risorse CSV degli strumenti](https://adobe-consulting-services.github.io/acs-aem-tools/features/csv-asset-importer/index.html) ACS AEM estrae risorse dal file system e dai metadati di risorse da un file CSV per l’importazione delle risorse. L’API AEM Asset Manager viene utilizzata per importare le risorse nel sistema e applicare le proprietà dei metadati configurate. Idealmente, le risorse sono montate sul server tramite un montaggio di file di rete o tramite un&#39;unità esterna.
...
## Migrazione tra le istanze di AEM {#migrating-between-aem-instances}
...
1. Poiché state eseguendo la migrazione delle rappresentazioni insieme alle risorse, desiderate disattivare gli avviatori dei flussi di lavoro per DAM Update Asset.

1. Poiché i tag sono già stati caricati nell’istanza AEM di origine, potete crearli in un pacchetto di contenuto e installare il pacchetto sull’istanza di destinazione.

1. Per spostare le risorse da un’istanza di AEM a un’altra sono consigliati due strumenti:
...
1. Seguite le istruzioni per [attivare le risorse](#activating-assets) documentate per la migrazione iniziale ad AEM.

1. Come per la nuova migrazione, il caricamento di un’istanza di pubblicazione singola e la duplicazione è più efficiente rispetto all’attivazione del contenuto su entrambi i nodi. Consultate [Clonazione della pubblicazione.](#cloning-publish)

1. Dopo aver completato la migrazione, riattivate i lanciatori per i flussi di lavoro DAM Update Asset per supportare la generazione di rappresentazioni e l&#39;estrazione di metadati per un utilizzo quotidiano del sistema.
