...
Content Hub consente di creare nuovi contenuti con Adobe Express. Puoi modificare i contenuti esistenti con strumenti facili da usare, produrre varianti in linea con il brand con modelli ed elementi del brand e creare nuovi contenuti con le più recenti funzionalità GenAI di Adobe Firefly.
...
## Domande frequenti {#faqs-edit-images-content-hub}

### Chi può modificare le immagini in AEM Assets Content Hub?

Gli utenti che dispongono dei diritti per accedere ad Adobe Express e AEM Assets Content Hub e che dispongono dei diritti per eseguire il remix delle risorse in nuove varianti possono modificare le immagini utilizzando AEM Assets Content Hub.

### Come si modifica un’immagine utilizzando Adobe Express in AEM Assets Content Hub?

Per modificare un&#39;immagine in AEM Assets Content Hub, fai clic su **Apri in Adobe Express** nella scheda delle risorse dell&#39;immagine oppure apri i dettagli dell&#39;immagine e fai clic sul logo Adobe Express. In questo modo viene caricato l’editor di Adobe Express incorporato in AEM Assets Content Hub, che consente di apportare le modifiche senza uscire dalla piattaforma.

### Quali funzioni di modifica sono fornite da Adobe Express in AEM Assets Content Hub?

Adobe Express offre diverse funzioni di modifica delle immagini in AEM Assets Content Hub, tra cui ridimensionamento delle immagini, rimozione o modifica del colore di sfondo, ritaglio delle immagini e combinazione con immagini o testo generati dall’intelligenza artificiale.

### Quali formati di file è possibile salvare le immagini modificate come in AEM Assets Content Hub?

Le immagini modificate possono essere salvate in formato PNG (buona qualità), JPG (adatto a file di piccole dimensioni) o PDF (adatto a documenti) in AEM Assets Content Hub.

### Quali campi devo compilare quando salvo un’immagine modificata in AEM Assets Content Hub?

Quando salvi un&#39;immagine modificata in AEM Assets Content Hub, devi specificare un nome per la risorsa nel campo **Salva con nome** e un nome della campagna nel campo **Nome campagna**. Adobe consiglia inoltre di specificare valori in campi aggiuntivi, come Parole chiave, Canali, Intervallo temporale e Area geografica, per migliorare la ricercabilità e l’organizzazione.

### Posso salvare le modifiche come nuova risorsa in AEM Assets Content Hub?

Sì, dopo la modifica, puoi fare clic su **Salva come nuova risorsa** per salvare le modifiche come nuova risorsa in AEM Assets Content Hub.

### Gli amministratori possono personalizzare i campi durante il caricamento delle risorse in AEM Assets Content Hub?

Sì, gli amministratori possono configurare quali campi sono obbligatori o facoltativi quando aggiungono risorse ad AEM Assets Content Hub, ad esempio il nome della campagna, le parole chiave e i canali, in base alle esigenze aziendali. Utilizza la scheda **Importa** nell&#39;interfaccia utente di configurazione per configurare i campi.
