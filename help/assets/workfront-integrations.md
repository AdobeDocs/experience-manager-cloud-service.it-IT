...
Adobe di offerte a [integrare [!DNL Workfront] e [!DNL Adobe Experience Manager Assets] in formato nativo (con supporto di Assets Essentials e Assets as a Cloud Service), utilizzando il connettore Workfront per AEM o utilizzando Workfront per Experience Manager un connettore avanzato](https://experienceleague.adobe.com/docs/workfront/using/documents/wf-aem-integrations/wf-aem-essentials/aem-asset-integrations.html). In caso di integrazione nativa, non è necessario un connettore per integrare le due soluzioni.

>[!NOTE]
>
>Adobe non supporta l’utilizzo in parallelo di Workfront per il connettore AEM o Workfront per Experience Manager l’integrazione di connettori e Experienci Manager avanzati.
...
| Ricerca di cartelle AEM in Workfront | Il selettore AEM Assets in Workfront consente ricerche full-text per le cartelle. | ✓ | No | ✓ | ✓ |
...
| Crea automaticamente un documento personalizzato Forms in Workfront | Allegare moduli personalizzati a documenti, attività e problemi di Workfront utilizzando i flussi di lavoro AEM. | No | Aggiungere manualmente il modulo personalizzato, quindi la sincronizzazione automatica funziona | ✓ | No |
...
| Visualizzazione in tempo reale in Workfront per metadati mappati a AEM | Visualizza i metadati mappati aggiornati da AEM nei pannelli Dettagli documento e Riepilogo documento di Workfront. | ✓ | No | No | ✓ |
| Invio in tempo reale di metadati Workfront aggiornati a AEM | Aggiorna automaticamente i metadati Workfront mappati in AEM senza dover ripetere l&#39;invio di una risorsa o di una nuova versione di una risorsa. | ✓ | No | No | ✓ |
...
| Aggiornamenti AEM metadati con nuove versioni | È possibile effettuare una configurazione in AEM per determinare se una risorsa con nuove versioni in Workfront invia anche le modifiche apportate ai suoi metadati. | No | No | ✓ | No |
...
| Imposta stato su oggetti Workfront | Impostare gli stati degli oggetti di Workfront in base a condizioni configurabili tramite flussi di lavoro AEM | No | No | ✓ | Prossima |
| Pubblicare risorse in AEM Publish Environment o Brand Portal | Consenti agli utenti di Workfront di pubblicare automaticamente le risorse collegate in un ambiente di pubblicazione AEM o Brand Portal. | No | No | ✓ | Prossima |
