...
Quando un utente richiede un download contenente cartelle o raccolte, AEM esegue una stima rapida del numero di elementi (Assets, Cartelle o Rappresentazioni) sotto le cartelle o le raccolte scaricate, per garantire che il download richiesto rientri nei limiti supportati. Per impostazione predefinita, i download contenenti più di 50.000 elementi sono bloccati e AEM visualizza il messaggio `The selected items are larger than the configured maximum download limit`.

È possibile aumentare il limite delle dimensioni di download aggiungendo una configurazione OSGI mostrata di seguito al codice dell&#39;applicazione e [distribuendola tramite una pipeline Cloud Manager](https://experienceleague.adobe.com/it/docs/experience-manager-cloud-service/content/implementing/deploying/configuring-osgi):
