...
Enterprise Manager Assets consente di modificare contemporaneamente i metadati di più risorse, in modo da poter rapidamente propagare le modifiche comuni ai metadati delle risorse in blocco. Utilizza la [!UICONTROL Proprietà] per modificare le proprietà dei metadati in un valore comune o per aggiungere o modificare tag. Per personalizzare la pagina Proprietà metadati, ad esempio aggiungere, modificare, eliminare proprietà di metadati, utilizza l’editor schema.

>[!NOTE]
>
>I metodi di modifica collettiva funzionano per le risorse disponibili in una cartella o in una raccolta. Per le risorse disponibili tra le cartelle o che corrispondono a un criterio comune, è possibile [aggiorna in blocco i metadati dopo la ricerca](/help/assets/search-assets.md#metadata-updates).
...
1. Dalla barra degli strumenti, tocca o fai clic su **[!UICONTROL Proprietà]** per aprire [!UICONTROL Proprietà] per le risorse selezionate.
   ...
   >[!NOTE]
   >
   >Quando selezioni più risorse, per le risorse viene selezionato il modulo principale più basso. In altre parole, il [!UICONTROL Proprietà] visualizza solo i campi di metadati comuni in tutta la pagina [!UICONTROL Proprietà] pagine di tutte le singole risorse.
...
1. Per visualizzare l’editor di metadati per una risorsa specifica, annulla la selezione delle risorse rimanenti nell’elenco. I campi dell’editor di metadati sono compilati con i metadati per la risorsa in questione.
   ...
   >[!NOTE]
   >
   >* In [!UICONTROL Proprietà] Per rimuovere le risorse dall’elenco delle risorse, annulla la selezione. Per impostazione predefinita, nell’elenco delle risorse sono selezionate tutte le risorse. I metadati per le risorse rimosse dall’elenco non vengono aggiornati.
...
* DEX - [Esploratore descrizione di Optima SC](https://www.optimasc.com/products/dex/index.html) è una raccolta di strumenti per la gestione di metadati e file per i sistemi operativi Windows.
