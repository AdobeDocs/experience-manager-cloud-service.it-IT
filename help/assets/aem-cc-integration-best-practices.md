...
description: Le best practice integrano una distribuzione Experience Manager  con Adobe Creative Cloud per semplificare i flussi di lavoro di trasferimento delle risorse e ottenere la massima efficienza.
contentOwner: AG
mini-toc-levels: 1
...
* [AEM Assets Brand Portal](https://helpx.adobe.com/it/experience-manager/brand-portal/user-guide.html) per un componente aggiuntivo SaaS configurabile ai AEM Assets per la pubblicazione delle risorse.
...
* [Interfaccia](/help/assets/manage-digital-assets.md) Web AEM Assets con aree per soggetti esterni protetti dalla configurazione di AEM Access Control e con le necessarie regolazioni IT/di configurazione della rete, per consentire a questi utenti esterni di accedere ad AEM.
...
* Quando le risorse di Adobe stock vengono salvate in AEM, diventano AEM Assets regolari e il file binario viene salvato nell’archivio di AEM. Alcuni metadati relativi ad Adobe Stock vengono salvati per la risorsa in AEM, altrimenti il processo di assimilazione sarà simile a quello di qualsiasi altro file. Ad esempio, se i tag avanzati sono attivi, al momento del salvataggio vengono aggiunti a tali risorse.
...
Le risorse da Adobe Stock con licenza e salvate in AEM diventano disponibili per i team più grandi che accedono alla distribuzione di AEM Assets, mentre le risorse per la licenza di Adobe Stock mediante il pannello Creative Cloud Libraries le rendono disponibili per se stesse solo per impostazione predefinita nel loro account Creative Cloud.
...
I AEM Assets supportano due tipi di utenti in base al loro accesso alla distribuzione dei AEM Assets. In genere, gli utenti all&#39;interno della rete aziendale (firewall) hanno accesso diretto a DAM. Gli altri utenti esterni alla rete aziendale non avrebbero accesso diretto. Il tipo di utente determina quali integrazioni possono essere utilizzate dal punto di vista tecnico.

#### Utenti creativi con accesso diretto a DAM {#creative-users-with-direct-access-to-dam}

In genere, i team creativi interni o le agenzie/i professionisti creativi caricati sulla rete interna hanno accesso all’istanza DAM, incluso l’accesso AEM. AEM e l’infrastruttura di rete possono essere configurate per consentire l’accesso diretto a soggetti esterni, in genere organizzazioni affidabili come le agenzie che lavorano per un cliente, per accedere ad AEM tramite rete, ad esempio tramite VPN o elenco Consentiti di  IP.
...
* Per caricare alcuni file da una singola cartella, trascinateli direttamente nell’interfaccia Web o usate l’opzione Crea nell’interfaccia Web dei AEM Assets.
...
Evitate di utilizzare l&#39;app desktop AEM per gestire le risorse direttamente nella condivisione di rete di AEM Assets. Ad esempio, evitate di utilizzare l&#39;app desktop AEM per spostare/copiare più file. Utilizzate, invece, l&#39;interfaccia utente Web AEM Assets per trascinare le cartelle dal Finder/Esplora risorse alla condivisione di rete o utilizzate la funzione di caricamento delle cartelle AEM Assets.
