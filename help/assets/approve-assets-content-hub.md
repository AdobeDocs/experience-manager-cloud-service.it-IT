...
### Qual è lo scopo di approvare le risorse per AEM Assets Content Hub in Experience Manager as a Cloud Service? {#approving-assets-content-hub}

L’approvazione delle risorse garantisce che solo le versioni più recenti e approvate siano disponibili per l’utilizzo in AEM Assets Content Hub, mantenendo una rigorosa coerenza del marchio per tutti i canali e le applicazioni. Questo processo controllato semplifica la gestione delle risorse per i brand manager e gli addetti al marketing.

### Quali sono i prerequisiti necessari per approvare le risorse per AEM Assets Content Hub?
...
### Come si approva una singola risorsa utilizzando la vista Assets in AEM as a Cloud Service in modo che sia disponibile in AEM Assets Content Hub?

Seleziona la risorsa, fai clic su **Dettagli** nella barra degli strumenti, passa alla scheda **Base**, scegli **Approvato** dall&#39;elenco a discesa **Stato**, quindi fai clic su **Salva**. La risorsa è disponibile in AEM Assets Content Hub.

### È possibile approvare in blocco le risorse per AEM Assets Content Hub? In caso affermativo, in che modo?

Sì, le risorse possono essere approvate in blocco. Nella vista Assets, seleziona più risorse, fai clic su **Modifica in blocco metadati**, seleziona **Approvato** nel campo **Stato** in Proprietà, quindi fai clic su **Salva**. Tutte le risorse selezionate sono disponibili in AEM Assets Content Hub.

### Come funziona il processo di approvazione delle risorse in AEM Assets Content Hub? {#asset-approval-content-hub}

Se l’opzione di approvazione automatica è abilitata, le risorse caricate tramite AEM Assets Content Hub sono automaticamente disponibili. Se è disabilitata, le risorse caricate vengono inserite nella cartella **idratated-assets** in Assets as a Cloud Service e devi modificarne manualmente lo stato in massa in **Approvato** per renderle visualizzate in Content Hub.

### Cos’è il campo Target di approvazione nella vista AEM Assets e come influisce sulla pubblicazione delle risorse?
...
### Cosa succede se non visualizzi il campo Target di approvazione nella pagina Visualizza dettagli risorsa di AEM Assets?
...
### Come puoi automatizzare l’approvazione per le nuove risorse acquisite nella visualizzazione Amministratore di AEM Assets?
...
### Chi può accedere alle risorse approvate in AEM Assets Content Hub e quali controlli sono attivi?

Le risorse approvate sono disponibili per gli utenti che fanno parte della stessa organizzazione in AEM Assets Content Hub. Controlli rigorosi garantiscono l’accessibilità solo delle versioni più recenti e approvate, contribuendo a mantenere la coerenza e la sicurezza del marchio.
