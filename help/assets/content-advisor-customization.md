---
title: Personalizza Contenuto verificato
description: Utilizzare le funzioni per personalizzare Contenuto verificato all'interno dell'applicazione.
role: Admin, User
...
# Personalizzazioni di Contenuto verificato {#content-advisor-customization}

Contenuto verificato consente di personalizzare vari componenti in base a preferenze, requisiti o esigenze funzionali. È possibile personalizzare i seguenti componenti [Contenuto advisor Microsoft Front End](#overview-asset-selector.md):
...
Contenuto verificato consente di controllare l&#39;utilizzo di una risorsa scaduta. Puoi personalizzare la risorsa scaduta con un distintivo **In scadenza** che ti aiuterà a conoscere in anticipo le risorse che scadranno entro 30 giorni dalla data corrente. Inoltre, questo può essere personalizzato in base al requisito. Puoi anche consentire la selezione di una risorsa scaduta nell’area di lavoro o viceversa. La personalizzazione di una risorsa scaduta può essere eseguita utilizzando alcuni snippet di codice in vari modi:
...
Puoi personalizzare l’utilizzo di una risorsa scaduta per renderla selezionabile o non selezionabile. È possibile personalizzare se si desidera consentire o meno il trascinamento di una risorsa scaduta nell&#39;area di lavoro di Contenuto verificato. A questo scopo, utilizza i seguenti parametri per rendere una risorsa non selezionabile sull’area di lavoro:
...
Contenuto verificato consente di aggiungere un filtro di selezione tag. Supporta un gruppo di tag che combina tutti i tag pertinenti a un particolare gruppo di tag. Inoltre, ti consente di selezionare altri tag corrispondenti alla risorsa che stai cercando. Inoltre, puoi anche impostare i gruppi di tag predefiniti sotto il filtro di chiamata contestuale, che vengono utilizzati principalmente da te in modo che siano accessibili da te in movimento.
...
## Carica in Contenuto verificato {#upload-in-content-advisor}

È possibile caricare file o cartelle in Content Advisor dal file system locale. Per caricare i file utilizzando il file system locale, in genere è necessario utilizzare una funzione di caricamento fornita da una micro applicazione front-end di Content Advisor. `upload` Vari snippet di codice necessari per richiamare il caricamento in Content Advisor includono:
...
*Altre proprietà includono `metadataSchema`, `onMetadataFormChange`, `targetUploadPath`, `hideUploadButton`, `onUploadStart`, `importSettings` `onUploadComplete`, `onFilesChange`,`uploadingPlaceholder`*. Per ulteriori informazioni, vedere [Proprietà di Content Advisor](/help/assets/content-advisor-properties.md).
