...
* SITES-19614: [Xwalk] Paginazione/scorrimento infinito per fogli di calcolo.
...
* SITES-20405: [Xwalk] - Supporto di mimeType per la compressione dei campi.
...
* SITES-20653: [Xwalk] - Aggiunta di experiment-start-date e -end-date.
* SITES-20656: [BE] - Nuova opzione per corrispondenza di maiuscole/minuscole durante la sostituzione di una stringa.
* SITES-20666: [Xwalk] - Gli esperimenti devono essere disattivati per impostazione predefinita durante l’authoring.
...
* SITES-21550: [Xwalk] - Metadati personalizzati: campi numero, data, datetime e ora.
...
* SITES-19554: [Xwalk] - Editor fogli di calcolo: impossibile svuotare una cella.
...
* SITES-20496: opzione nessuna proprietà [Xwalk] quando si seleziona il foglio di calcolo in Amministrazione sito.
...
* Alcuni utenti potrebbero riscontrare problemi durante la creazione di moduli su `Cloud Service version 2024.5.16461`Tuttavia, quando un utente crea un modulo, nella finestra di dialogo di creazione viene visualizzato il seguente messaggio di errore:

  `A server error occurred. Try again after sometime.`

  Il problema si verifica perché l’autore pubblica il modulo senza prima pubblicare il modello utilizzato nel modulo, il che si traduce nell’aggiunta del `jcr:uuid` e altre proprietà protette e generate dal sistema `<template-path>/initial/jcr:content` nodo. Questo causava errori nella successiva creazione del modulo.

  Per risolvere il problema, effettua le seguenti operazioni:

   1. Verificare che il modello utilizzato nel modulo non disponga di `jcr:uuid` e altre proprietà protette generate dal sistema nel percorso `<template-path>/initial/jcr:content` nodo.
