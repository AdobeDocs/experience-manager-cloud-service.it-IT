...
| [cattivi](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.form.rte.plugins.MiscToolsPlugin) | `specialchars`, `sourceedit` | Gli strumenti vari consentono agli autori di immettere caratteri [](configure-rich-text-editor-plug-ins.md#spchar) speciali o modificare l’origine HTML. Potete inoltre aggiungere un [intervallo di caratteri](configure-rich-text-editor-plug-ins.md#definerangechar) speciali per definire un elenco personalizzato. |
...
| stili | `styles` | Supporto per lo stile con una classe CSS. [Aggiungete nuovi stili](configure-rich-text-editor-plug-ins.md#textstyles) di testo se desiderate aggiungere (o estendere) un intervallo personalizzato di stili da usare con il testo. |
| pedice | `subscript`, `superscript` | Estensioni nei formati di base, aggiunta di pedice e super-script. |
| tabella | `table`, `removetable`, `insertrow`, `removerow`, `insertcolumn`, `removecolumn`, `cellprops`, `mergecells`, `splitcell`, `selectrow`, `selectcolumns` | Consultate [Configurare gli stili](configure-rich-text-editor-plug-ins.md#tablestyles) di tabella per aggiungere stili personalizzati per intere tabelle o singole celle. |
| annulla | `undo`, `redo` | Dimensione cronologia delle operazioni di [annullamento e ripristino](configure-rich-text-editor-plug-ins.md#undohistory) . |
...
La [modalità di modifica dell’editor Rich Text e l’interfaccia](#editingmodes) fornita dagli autori determinano il percorso dei dettagli di configurazione quando si [attivano i plug-in](configure-rich-text-editor-plug-ins.md#activateplugin)dell’editor Rich Text. Le posizioni sono:
...
* `useFixedInlineToolbar`: È possibile impostare la barra degli strumenti dell’editor Rich Text in modo che sia fissa invece di essere mobile. Impostate questa proprietà booleana definita sul nodo RTE con sling:resourceType= `cq/gui/components/authoring/dialog/richtext` su `True`. Quando questa proprietà è impostata su `True`, la modifica RTF viene avviata sull&#39; `foundation-contentloaded` evento. Per evitare questo problema, impostare la proprietà `customStart` su `True` e attivare l’ `rte-start` evento per avviare la modifica dell’editor Rich Text. Quando questa proprietà è `true`, l’editor Rich Text non inizia a fare clic e questo è il comportamento predefinito.
...
Se l&#39;opzione è un pop-over (contenente alcune funzioni di un plug-in), viene specificata come `#PluginName` (ad esempio, `#format`).

I separatori (`|`) tra un gruppo di opzioni possono essere specificati con `-`.

Il nodo a comparsa nella modalità in linea o a schermo intero contiene un elenco dei pop-up in uso. Ogni nodo secondario sotto il `popovers` nodo viene denominato in base al plug-in (ad esempio, format). La proprietà &#39;items&#39; contiene un elenco delle funzioni del plug-in (ad esempio, format#bold).
...
* Le funzionalità RTE sono supportate solo nelle finestre di dialogo dei [!DNL Experience Manager] componenti. L&#39;editor Rich Text non è supportato nelle procedure guidate o nei moduli di base.
