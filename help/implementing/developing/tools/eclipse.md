...
* Scaricate e installate [Eclipse IDE per gli sviluppatori](https://www.eclipse.org/downloads/packages/)Java Enterprise.
* Configurate l&#39;installazione dell&#39;eclisse per garantire che possiate disporre di almeno 1 gigabyte di memoria heap modificando il file di `eclipse.ini` configurazione come descritto nelle domande frequenti [Eclipse](https://wiki.eclipse.org/FAQ_How_do_I_increase_the_heap_size_available_to_Eclipse).
...
1. In **Nome** immettete `AEM Developer Tools`.
1. In **Location** copiate l’URL di installazione.
1. Fate clic su **Aggiungi**.
1. Controllate i plug-in **AEM** e **Sling** .
1. Fai clic su **Avanti**.
1. Nella finestra **Dettagli** installazione, fate di nuovo clic su **Avanti** .
1. Accettate i contratti di licenza e fate clic su **Fine**.
1. Fate clic su **RestartNow** per riavviare Eclipse.

## La prospettiva AEM {#the-aem-perspective}

In Eclipse una prospettiva determina le azioni e le viste disponibili all’interno di una finestra e abilita l’interazione orientata alle attività con le risorse in Eclipse. Per ulteriori dettagli sulla prospettiva, consultate la documentazione di [Eclipse.](https://help.eclipse.org)
...
1. Dalla barra dei menu Eclipse, selezionate **Finestra** -> **Prospettiva** -> **Apri prospettiva** -> **Altro**.
1. Selezionate **AEM** nella finestra di dialogo e fate clic su **Apri**.
...
1. Scegliete `com.adobe.granite.archetypes : sample-project-archetype : <highest-number>` dal menu, quindi fate clic su **Avanti**.
...
1. Click **Finish**. La struttura del progetto viene creata.
   ...
   >[!NOTE]
   >
   >In una nuova installazione (più precisamente, quando le dipendenze di colore non sono mai state scaricate) è possibile che il progetto venga creato con errori. In questo caso, seguire la procedura descritta in [Risoluzione della definizione](#resolving-invalid-project-definition)di progetto non valida.

## Come importare progetti esistenti {#how-to-import-existing-projects}

Potete usare la funzione **Nuovo progetto** per creare la struttura corretta:

1. Seguite le istruzioni per creare un progetto [Multi-Modulo di](#sample-multi-module-project) esempio e avrete a disposizione i seguenti progetti, che consentiranno una sana separazione dei problemi:

   * `PROJECT.ui.apps` per `/apps` e `/etc` il contenuto
...
1. Sostituite il contenuto del `PROJECT.ui.apps` progetto con le cartelle `apps` e `etc` le cartelle del pacchetto:

   1. Nel pannello Esplora progetti, aprite `PROJECT.ui.apps` > `src` > `main` > `content` > `jcr_root` > `apps`.
   1. Fate clic con il pulsante destro del mouse sulla `apps` cartella e scegliete **Mostra in** > **Esplora** sistema.
   1. Eliminate le cartelle `apps` e `etc` quelle che dovrebbero essere visualizzate e inserite qui le cartelle `apps` e `etc` il pacchetto di contenuti.
   1. In Eclipse, fai clic con il pulsante destro del mouse sul `PROJECT.ui.apps` progetto e scegli **Aggiorna**.

1. Quindi effettuate le stesse operazioni per la cartella `PROJECT.ui.content` e sostituite la cartella del contenuto con quella del pacchetto:

   1. Nel pannello Esplora progetti, aprite `PROJECT.ui.content` > `src` > `main` > `content` > `jcr_root` > `content`.
   1. Fare clic con il pulsante destro del mouse sulla cartella del contenuto più profondo e scegliere **Mostra in** -> **Esplora** sistema.
...
1. Ora dovete aggiornare i `filter.xml` file di questi due progetti in modo che corrispondano al contenuto del pacchetto di contenuti. Per questo, aprite il `META-INF/vault/filter.xml` file del pacchetto di contenuti in un editor di testo o codice separato.

   * Questo è un esempio dell’aspetto del `filter.xml` file:
...
1. Per quanto riguarda il contenuto del pacchetto suddiviso in due progetti, dovrete anche dividere queste regole di filtro in due e aggiornare di conseguenza i `filter.xml` file dei due progetti.

   1. In Eclipse, aprite `PROJECT.ui.apps/src/main/content/META-INF/filter.xml`.
   1. Sostituite il contenuto dell&#39; `<workspaceFilter>` elemento con le regole del pacchetto che iniziano con `/apps` e `/etc`
      * Esempio:
         ...
         ```
   1. Quindi aprite `PROJECT.ui.content/src/main/content/META-INF/filter.xml`.
   1. Sostituite le regole con quelle del pacchetto che iniziano con `/content`.
      * Esempio:
...
1. Fate clic sull’icona **Pulisci e pubblica** .
...
Se desiderate rigenerare un pacchetto dal progetto, fate clic con il pulsante destro del mouse sul progetto `PROJECT.ui.apps` o `PROJECT.ui.content` e scegliete **Esegui come** -> **Maven Install**.
...
1. Nel menu di scelta rapida, selezionate **Paradiso** -> **Aggiorna progetti**.
1. Verificare la **forza degli aggiornamenti di snapshot/release**.
1. Fai clic su **OK**.
