---
title: Comprendere la struttura del pacchetto di contenuti di un progetto
description: Scopri come definire correttamente le strutture dei pacchetti per la distribuzione in Adobe Experience Manager Cloud Service.
...
AEM richiede una separazione di **contenuto** e **codice**, il che significa che un singolo pacchetto di contenuti **non può** essere distribuito su aree scrivibili **sia** `/apps` che runtime (ad esempio `/content`, `/conf`, `/home`o altro) `/apps`del repository. Al contrario, l&#39;applicazione deve separare codice e contenuto in pacchetti discreti per la distribuzione in AEM.

La struttura del pacchetto descritta in questo documento è compatibile con le distribuzioni di sviluppo **** locali e con le distribuzioni di servizi AEM Cloud.
...
`/apps` e `/libs` sono considerate aree **immutabili** di AEM poiché non possono essere modificate (create, aggiornate, eliminate) dopo l’avvio di AEM (ad es. in fase di runtime). Qualsiasi tentativo di modificare un&#39;area immutabile in fase di esecuzione avrà esito negativo.
...
+ Il `ui.apps` pacchetto, o pacchetto di contenuto, contiene tutto il codice da distribuire e da distribuire solo a `/apps`. Gli elementi comuni del `ui.apps` pacchetto includono, tra l&#39;altro:
   + Pacchetti OSGi
      + `/apps/my-app/install`
   + Configurazioni OSGi
      + `/apps/my-app/config`
...
+ Il `ui.content` pacchetto, o pacchetto di codice, contiene tutto il contenuto e la configurazione. Gli elementi comuni del `ui.content` pacchetto includono, tra l&#39;altro:
   + Configurazioni basate sul contesto
      + `/conf`
   + Strutture di contenuto di base (cartelle di risorse, pagine principali dei siti)
   ...
   + Altri nodi legacy
      + `/etc`
   + ACL (autorizzazioni)
      + Qualsiasi `rep:policy` per qualsiasi percorso **non** in `/apps`
+ Il `all` pacchetto è un pacchetto contenitore che include SOLO i pacchetti `ui.apps` e `ui.content` come embeds. Il `all` pacchetto non deve avere **alcun contenuto** proprio, ma deve delegare tutta la distribuzione al repository ai relativi pacchetti secondari.
   ...
   Ad esempio, una struttura complessa di pacchetti di contenuto di distribuzione potrebbe essere simile alla seguente:
...
Per impostazione predefinita, Adobe Cloud Manager raccoglie tutti i pacchetti prodotti dalla build Maven, tuttavia, poiché il pacchetto contenitore (`all`) è il singolo artifact di distribuzione che contiene tutti i pacchetti di codice e contenuto, è necessario assicurarsi che sia distribuito **solo** il pacchetto contenitore (`all`). A questo scopo, gli altri pacchetti generati dalla build Maven devono essere contrassegnati con la configurazione del plug-in FileVault Content Package Maven di `<properties><cloudManagerTarget>none</cloudManageTarget></properties>`.
...
Questo è richiesto **** solo per i pacchetti Codice, ovvero per tutti i pacchetti contrassegnati con `<packageType>application</packageType>`.
...
I pacchetti di contenuto o codice vengono inseriti in una speciale cartella &quot;side-car&quot; e possono essere installati sia sull&#39;autore AEM che sulla pubblicazione AEM, o su entrambi, utilizzando la `<embeddeds>` configurazione del plug-in FileVault Maven. La `<subPackages>` configurazione non deve essere utilizzata.
...
+ La cartella di secondo livello rappresenta l’applicazione con `-packages` post-fisso sul nome della cartella. Spesso esiste una sola cartella di 2° livello in cui sono incorporati tutti i pacchetti secondari, tuttavia è possibile creare un numero qualsiasi di cartelle di 2° livello per rappresentare al meglio la struttura logica dell&#39;applicazione:
   ...
   >[!WARNING]
   >
   >Per convenzione, le cartelle incorporate in un sottopacchetto hanno un nome con il suffisso `-packages`. In questo modo il codice di distribuzione e i pacchetti di contenuto **non** vengono distribuiti nelle cartelle di destinazione di qualsiasi sottopacchetto `/apps/<app-name>/...` , con un comportamento di installazione distruttivo e ciclico.
...
+ La cartella di quarto livello contiene i pacchetti secondari e deve essere una delle seguenti:
   + `install` per effettuare l’installazione sia sull’autore **che** sulla pubblicazione AEM
   + `install.author` per eseguire l&#39;installazione **solo** sull&#39;autore di AEM
...
Se i pacchetti di terze parti si trovano nell&#39;archivio **degli artifact Maven pubblico di** Adobe, non è necessaria alcuna ulteriore configurazione per consentire ad Adobe Cloud Manager di risolvere gli artefatti.

Se i pacchetti di terze parti si trovano in un archivio **Maven** pubblico di terze parti, il repository deve essere registrato nel progetto `pom.xml` e incorporato seguendo il metodo [descritto sopra](#embeddeds). Se l&#39;applicazione/connettore di terze parti richiede pacchetti di codice e di contenuto, ciascuno di essi deve essere incorporato nelle posizioni corrette nel pacchetto contenitore (`all`).
...
Le strutture e l&#39;organizzazione del progetto descritte in questo articolo sono **completamente compatibili** con le istanze AEM di sviluppo locale.
...
I pacchetti di codice e di contenuto, distribuiti come pacchetti secondari, devono dichiarare un tipo di pacchetto di **applicazione** o di **contenuto**, a seconda del contenuto.
...
In ogni progetto che genera un pacchetto, **fatta eccezione** per il progetto contenitore (`all`), aggiungete `<cloudManagerTarget>none</cloudManagerTarget>` alla `<properties>` configurazione della dichiarazione del `filevault-package-maven-plugin` plug-in per assicurarvi che non **siano** distribuiti da Adobe Cloud Manager. Il pacchetto contenitore (`all`) deve essere il singolo pacchetto distribuito tramite Cloud Manager, che a sua volta incorpora tutti i pacchetti di codice e contenuto richiesti.
...
Nel `all` progetto `filter.xml` (`all/src/main/content/jcr_root/META-INF/vault/definition/filter.xml`), **includete** tutte le `-packages` cartelle che contengono pacchetti secondari da distribuire:
...
Nel progetto del reattore `pom.xml`, aggiungere tutte le necessarie direttive del repository di Maven di terze parti. La `<repository>` configurazione completa dovrebbe essere disponibile dal provider del repository di terze parti.
