...
* [SPA ](blueprint.md) Blueprintfor per i requisiti tecnici di un SPA
* [Guida introduttiva alla SPA in AEM con ](getting-started-react.md) Reactfor per una breve presentazione di un semplice SPA con React
* [Guida introduttiva a SPA in AEM con ](getting-started-angular.md) Angularper un rapido tour di un semplice SPA con Angular
...
Altri selettori personalizzati possono essere definiti e utilizzati come parte di un SPA sviluppato per l’SDK SPA AEM. Tuttavia, questo supporto richiede che il selettore `model` sia il primo e l&#39;estensione sia `.json` come richiesto da JSON Exporter.
...
1. Impostate un attributo (che può essere uno qualsiasi) sull’elemento wrapper contenitore contenente il testo HTML. Nel caso del progetto WKND SPA, si tratta di un elemento `<div>` e il selettore utilizzato è `data-rte-editelement`.
...
Per ulteriori informazioni sulla proprietà `editElementQuery` e sulla configurazione dell&#39;editor Rich Text, vedere [Configurare l&#39;Editor Rich Text.](/help/implementing/developing/extending/rich-text-editor.md)
