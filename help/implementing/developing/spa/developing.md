...
## Principi di sviluppo SPA per i AEM {#spa-development-principles-for-aem}

Lo sviluppo di applicazioni a pagina singola in AEM presuppone che lo sviluppatore front-end rispetti le best practice standard per la creazione di un SPA. Se uno sviluppatore front-end segue queste best practice generali e pochi principi AEM specifici, il SPA funzionerà con le funzioni di authoring dei [AEM e di creazione dei contenuti](introduction.md#content-editing-experience-with-spa).

* **[Portabilità](#portability)** - Come per qualsiasi componente, i componenti devono essere costruiti per essere il più possibile portatili. Il SPA deve essere realizzato con componenti portabili e riutilizzabili.
...
* **[Routing](#dynamic-routing)** dinamico - Il SPA è responsabile dell&#39;indirizzamento e AEM lo ascolta e recupera in base ad esso. Anche l&#39;indirizzamento deve essere dinamico.
...
Come per il rendering, anche tutti i routing devono essere dinamici. In AEM, [il SPA deve sempre possedere il routing](routing.md) e AEM ascoltare e recuperare il contenuto in base ad esso.
...
## Modelli SPA Design {#spa-design-models}

Se vengono seguiti i [principi di sviluppo di SPA in AEM](#spa-development-principles-for-aem) , il SPA funzionerà con tutte le funzioni AEM di authoring dei contenuti supportate.
...
<table>
 <tbody>
  ...
  <tr>
   <td>AEM viene utilizzato come CMS headless senza utilizzare il framework SDK di Editor <a href="/help/implementing/developing/spa/reference-materials.md">SPA.</a></td>
...
>[!NOTE]
>
>Anche se tutti i modelli sono supportati in AEM, solo implementando il terzo (e quindi seguendo i principi di sviluppo consigliati in AEM [](#spa-development-principles-for-aem)) gli autori dei contenuti potranno interagire con i SPA e modificarne il contenuto in AEM come sono abituati.

## Migrazione di SPA esistenti a AEM {#migrating-existing-spas-to-aem}

In genere, se il tuo SPA segue i [SPA Principi di sviluppo per AEM](#spa-development-principles-for-aem), il tuo SPA lavorerà in AEM e sarà modificabile utilizzando AEM SPA Editor.
...
Per ulteriori informazioni sull’utilizzo `MapTo` e la creazione di SPA per AEM in generale, consultate la Guida introduttiva per il framework scelto.
...
## AEM Architettura e SPA {#aem-architecture-and-spas}
...
* **AEM Author**
   ...
   1. Quando l&#39;applicazione SPA viene eseguita, viene richiesto il JSON, che consente all&#39;app di creare il DOM della pagina, inclusi gli `cq-data` attributi.
