...
Verificare che sia sempre disponibile un host virtuale corrispondente a ServerAlias `\*.local` nonché localhost, necessari per i processi di Adobe interni.
...
3. Verifica che il sottoinsieme dei file di configurazione dell’SDK di Dispatcher, destinati a essere immutabili come descritto in [Sezione struttura file](##flexible-mode-file-structure), non è stato modificato.
