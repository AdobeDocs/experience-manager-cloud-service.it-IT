...
* Ora chi usa Cloud Service può visualizzare i rapporti SLA (Service Level Agreement) in Cloud Manager. Questa funzionalità di visualizzazione sarà disponibile nei prossimi mesi.
Consulta [Rapporti SLA](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/using-cloud-manager/sla-reporting.html).

* Il tipo e la gravità di IndexType e delle regole di qualità `IndexDamAssetLucene` sono stati modificati. Entrambe queste regole sono ora bug di blocco *gravità*.

* Sono state introdotte nuove regole di qualità dell’indice Oak per coprire le configurazioni asincrone e Tika.
...
* SonarQube leggeva inutilmente i dati della cronologia Git. Su basi di codice di grandi dimensioni, questa lettura potrebbe comportare una penalizzazione inutile delle prestazioni della build.
...
* Lo stato Aggiornamento disponibile non deve essere visualizzato quando l’ultima versione è precedente alla versione corrente.

* L’onboarding iniziale non riusciva per le nuove organizzazioni con nomi lunghi.
