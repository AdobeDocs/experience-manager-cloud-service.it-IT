...
* Gli ambienti all’interno di un programma sandbox vengono sospesi quando non viene rilevata alcuna attività per un certo periodo di tempo. Questo stato non viene osservato in Cloud Manager. Tuttavia, lo stato può essere osservato tramite la Console per sviluppatori. Questa funzionalità verrà affrontata in una versione futura.

* Il collegamento alla Console per sviluppatori direttamente da Cloud Manager non mostra l’opzione per ibernare/riattivare l’ambiente di un programma sandbox. Per risolvere questo problema, una volta nella Console per sviluppatori, aggiungi il pattern `#release-cm-p1234-e5678` alla fine dell’URL, dove *1234* è l’ID del programma e *5678* è l’ID dell’ambiente. Questa funzionalità verrà affrontata in una versione futura.
