...
>[!NOTE]
>
>Per ulteriori informazioni sulle regole per la qualità del codice personalizzato eseguite da [!UICONTROL Cloud Manager], consulta il documento [Regole per la qualità del codice personalizzato](/help/implementing/cloud-manager/custom-code-quality-rules.md).
...
>[!NOTE]
>
>Per quanto sia una best practice rendere l’annotazione `@SuppressWarnings` più specifica possibile, ossia annotare solo l’istruzione o il blocco specifico causa del problema, è possibile procedere all’annotazione a livello di classe.

>[!NOTE]
>Sebbene non esista un esplicito passaggio per i test di sicurezza, durante il passaggio di qualità del codice vengono valutate alcune regole per la qualità del codice relative alla sicurezza. Per ulteriori informazioni sulla sicurezza in Cloud Service, consulta il documento [Panoramica sulla sicurezza di AEM as a Cloud Service](/help/security/cloud-service-security-overview.md).
