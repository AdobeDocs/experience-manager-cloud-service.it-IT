...
Per configurare queste impostazioni è necessario determinare la necessità di configurare `CNAME` o un record Apex affinché il nome di dominio personalizzato punti al nome di dominio di Cloud Manager. Le sezioni riportate di seguito aiutano a determinare il tipo di record appropriato per la configurazione DNS in uso.
