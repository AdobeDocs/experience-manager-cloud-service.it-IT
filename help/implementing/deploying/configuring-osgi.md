...
1. **Valori in linea**, valori inseriti a livello di codice nella configurazione OSGi e memorizzati in Git. Esempio:
...
1. **Valori specifici dell’ambiente**, valori che variano tra ambienti di sviluppo e che pertanto non possono essere oggetto di targeting accurato tramite la modalità di esecuzione (poiché esiste un singolo `dev` in Adobe Experience Manager as a Cloud Service). Esempio:
   ...
   Un singolo file di configurazione OSGi può utilizzare insieme qualsiasi combinazione di questi tipi di valori di configurazione. Esempio:
...
1. In questa cartella di configurazione, crea un `<PID>.cfg.json` file. Il PID è l’identità persistente del componente OSGi. In genere è il nome completo della classe dell’implementazione del componente OSGi. Esempio:
...
1. Accedi alla console web AEM di Quickstart Jar dell’SDK dell’AEM come utente amministratore
...
I valori in linea sono formattati come coppie nome-valore standard, seguendo la sintassi JSON standard. Esempio:
