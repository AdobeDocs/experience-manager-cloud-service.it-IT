...
* `itemid`: percorso JCR da aggiornare
* `itemprop`: proprietà JCR in fase di aggiornamento
* `itemtype`: tipo di valore JCR della proprietà da aggiornare
...
A `patch` la chiamata si verifica quando aggiorni il contenuto in una finestra di dialogo all’interno dell’app. In questo modo il contenuto all’interno della pagina dell’app viene aggiornato come patch JSON al contenuto esistente.

Il payload include il percorso del contenuto nella pagina e la patch JSON della modifica da apportare.

### Payload di esempio {#patch-payload}
...
### Risposta di esempio {#patch-response}
