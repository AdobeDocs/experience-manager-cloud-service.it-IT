...
Crea una configurazione cloud che connette AEM al provider di servizi di traduzione. L&#39;AEM include la capacità di [connessione a Microsoft® Translator](connect-ms-translator.md) per impostazione predefinita.
...
Quando si associa una pagina a una configurazione cloud, i discendenti della pagina ereditano tale associazione. Ad esempio, se hai associato `/content/wknd/language-masters/en/magazine` con un framework di integrazione della traduzione, il `magazine` pagina e le pagine secondarie sottostanti vengono tradotte in base al framework.
