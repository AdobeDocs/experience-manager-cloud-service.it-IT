...
È possibile creare [pagine di prodotti e categorie](../authoring/multi-template-usage.md) per un sottoinsieme specifico di categorie o prodotti di un catalogo.
...
Un approccio per farlo potrebbe essere quello di collegare un ambiente di staging al backend di e-commerce di produzione e configurarlo per utilizzare il nuovo formato URL. In seguito, ottieni la [mappa del sito del prodotto generata dal generatore di sitemap dei prodotti CIF](../../overview/seo-and-url-management.md) sia per stage che per l&#39;ambiente di produzione e utilizzali per creare una [mappa di riscrittura httpd Apache](https://httpd.apache.org/docs/2.4/rewrite/rewritemap.html). Questa mappa di riscrittura può quindi essere distribuita in Dispatcher insieme al rollout del nuovo formato URL.
