...
* Prima di seguire queste istruzioni è necessario concedere l’accesso come amministratore all’Admin Console
* Tieni presente che esistono alcuni modi diversi di creare utenti e gruppi in IMS.  Per informazioni su tutte le opzioni disponibili, consulta [Supporto IMS per Adobe Experience Manager as a Cloud Service](/help/security/ims-support.md).  Solo i metodi di caricamento in blocco di Admin Console sono descritti qui
* IMS supporta tre tipi di identità: **Adobe ID**, **Enterprise ID** e **Federated ID**. Le istruzioni in questa pagina sono fornite solo per **Adobe ID**.  Se devi utilizzare Enterprise ID o Federated ID, consulta la [documentazione Admin Console](https://helpx.adobe.com/it/enterprise/using/admin-console.html) completa e la documentazione specifica per il [caricamento di Admin Console Bulk Group](https://helpx.adobe.com/it/enterprise/using/user-groups.html) e il [caricamento di Admin Console Bulk User](https://helpx.adobe.com/enterprise/using/bulk-upload-users.html).  Le specifiche per i file di caricamento sono leggermente diverse per questi due tipi di identità
...
* Modifica il file utente ausiliario
