...
1. Se non viene trovata un’e-mail sul sistema Adobe Identity Management System (IMS) per l’ID organizzazione utilizzato (o, se non è possibile recuperare l’ID IMS), l’utente o il gruppo viene migrato ma non mappato.
...
1. Se nell&#39;istanza di AEM Cloud Service di destinazione esiste un utente con lo stesso nome utente (rep:principalName) di uno degli utenti nell&#39;istanza di AEM di origine, la migrazione dell&#39;utente o del gruppo non viene eseguita.
...
* Se l&#39;impostazione **Cancella contenuto esistente nell&#39;istanza Cloud prima dell&#39;acquisizione** è impostata, gli utenti già trasferiti nell&#39;istanza Cloud Service verranno eliminati. Viene eliminato anche l’intero archivio esistente e viene creato un nuovo archivio in cui viene acquisito il contenuto. Questa azione reimposta anche tutte le impostazioni, incluse le autorizzazioni sull&#39;istanza di Cloud Service di destinazione, ed è true per un utente amministratore aggiunto al gruppo **amministratori**. L&#39;utente amministratore deve essere letto al gruppo **amministratori** per recuperare il token di accesso per CTT.

* Adobe consiglia di rimuovere tutti gli utenti esistenti dall’istanza Cloud Service AEM di destinazione prima di eseguire CTT con mappatura utenti. Questa azione è necessaria per evitare conflitti tra la migrazione degli utenti dall’istanza AEM di origine all’istanza AEM di destinazione. Possono verificarsi conflitti durante l’acquisizione se lo stesso utente esiste nell’istanza AEM di origine e nell’istanza AEM di destinazione.
...
* Se AEM Cloud Service ha un utente con un nome utente diverso ma lo stesso indirizzo e-mail di un utente nell’istanza AEM di origine e la mappatura utente è abilitata, viene registrato un messaggio di errore. Inoltre, l’utente AEM di origine non viene trasferito, in quanto nel sistema di destinazione è consentito un solo utente con un determinato indirizzo e-mail.

* Se due utenti nell’istanza AEM di origine hanno lo stesso indirizzo e-mail e la mappatura utenti è abilitata, viene registrato un messaggio di errore. Inoltre, viene trasferito uno degli utenti AEM di origine perché nel sistema di destinazione è consentito un solo utente con un determinato indirizzo e-mail.
