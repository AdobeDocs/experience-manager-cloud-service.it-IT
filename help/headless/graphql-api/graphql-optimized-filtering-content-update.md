...
description: Scopri come aggiornare i frammenti di contenuto per il filtro ottimizzato per GraphQL in Adobe Experience Manager as a Cloud Service per la distribuzione di contenuti headless.
...
Assicurati di disporre almeno della versione 2023.1.0 di AEM as a Cloud Service.
...
1. Abilita l’aggiornamento impostando le seguenti variabili per l’istanza tramite l’interfaccia utente di Cloud Manager:
   ...
   <table style="table-layout:auto">
    <tbody>
     ...
     <tr>
      ...
      <td> </td>
      <td>Variabile </td>
      <td>Dimensione del batch per il salvataggio del numero di frammenti di contenuto dopo la migrazione.<br>Questo è rilevante per quanti CF verranno salvati nell'archivio in un batch e può essere utilizzato per ottimizzare il numero di scritture nell'archivio. </td>
     </tr>
     <tr>
      <td>5</td>
     ...
     <tr>
      ...
      <td> </td>
      <td>Variabile </td>
      <td>Intervallo (secondi) per elaborare i frammenti di contenuto rimanenti fino al limite successivo<br>Questo intervallo viene anche considerato come un tempo di attesa prima di avviare il processo, nonché un ritardo tra l'elaborazione di ogni successivo numero di CF CF CF_MIGRATION_LIMIT.<br>(*)</td>
     </tr>
    </tbody>
   </table>
   ...
   >Esempio:
   >
   >* Totale frammenti di contenuto = 20.000
   ...
   >
   >Devi anche sapere che questa è solo la *minimo* il tempo necessario per completare il processo e non include il tempo di I/O. Il tempo effettivamente impiegato potrebbe essere notevolmente superiore a questa stima.

1. Monitora l’avanzamento e il completamento dell’aggiornamento.

   A questo scopo, monitora i registri sull’authoring e Golden-Publish da:

   * `com.adobe.cq.dam.cfm.impl.upgrade.UpgradeJob`

      * Registri dell’autore; ad esempio:
         ...
         ```
   * Registri di pubblicazione dorati; ad esempio:
...
1. Disattiva la procedura di aggiornamento.
   ...
   Al termine della procedura di aggiornamento, reimposta la variabile di ambiente cloud `CF_MIGRATION_ENABLED` a &#39;0&#39;, per attivare il riciclaggio di tutti i pod.
   ...
   >[!NOTE]
   >
   >Ciò è particolarmente importante per il livello di pubblicazione, in quanto l’aggiornamento del contenuto viene eseguito solo su golden-publish e, quando si ricicla il pod, tutti i normali pod di pubblicazione si basano su golden-publish.
...
1. Verificare il completamento della procedura di aggiornamento.
   ...
   * Prima di completare la prima migrazione, `cfGlobalVersion` la proprietà non esisterà.
Pertanto, la presenza di questa proprietà, sul nodo JCR `/content/dam` con un valore di `1`, conferma il completamento della migrazione.
   ...
   * Puoi anche controllare le seguenti proprietà sui singoli frammenti di contenuto:
      ...
      * `indexedData` la struttura deve esistere
      ...
      >
      >Pertanto, si consiglia di eseguire la verifica tramite il browser dell’archivio per *almeno* un autore *e* un’istanza Publish.
...
* L’ottimizzazione delle prestazioni dei filtri di GraphQL sarà possibile solo dopo un aggiornamento completo di tutti i frammenti di contenuto (indicato dalla presenza di `cfGlobalVersion` proprietà per il nodo JCR `/content/dam`)

* Se i frammenti di contenuto vengono importati da un pacchetto di contenuti (utilizzando `crx/de`) dopo l’esecuzione della procedura di aggiornamento, tali frammenti di contenuto non verranno considerati nei risultati della query GraphQL, fino a quando la procedura di aggiornamento non viene eseguita nuovamente.