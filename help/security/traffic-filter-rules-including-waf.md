...
| CODEINJECTION | Code Injection | Code Injection è il tentativo di ottenere il controllo o danneggiare un sistema di destinazione attraverso comandi arbitrari di codice di applicazione mediante l’input dell’utente. |
...
Questa regola blocca un client per 5 minuti quando supera la media di 60 rich/sec (per POP CDN) negli ultimi 10 sec:
...
Blocca le richieste nel percorso /critical/resource per 60 secondi quando supera la media di 100 richieste da originare al secondo (per POP CDN) in un intervallo di tempo di 10 secondi:
