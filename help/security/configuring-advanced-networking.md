---
title: Configurare reti avanzate per AEM as a Cloud Service
...
# Configurare reti avanzate per AEM as a Cloud Service {#configuring-advanced-networking}

Questo articolo introduce le funzioni di rete avanzate disponibili in AEM as a Cloud Service. Queste funzioni includono il provisioning self-service e API di VPN, porte non standard e indirizzi IP in uscita dedicati.

Oltre a questa documentazione, è disponibile una serie di tutorial progettati per guidarti attraverso ciascuna delle opzioni di rete avanzate. Vedi [Rete avanzata](https://experienceleague.adobe.com/it/docs/experience-manager-learn/cloud-service/networking/advanced-networking).
...
* Un utente deve avere il ruolo **Amministratore** per aggiungere e configurare l&#39;infrastruttura di rete nel programma.
...
>[!TIP]
>
>Quando bisogna decidere tra uscita con porta flessibile e indirizzo IP in uscita dedicato, se non è necessario un indirizzo IP specifico si consiglia di scegliere l’uscita con porta flessibile. Il motivo è che Adobe può ottimizzare le prestazioni del traffico in uscita dalla porta flessibile.
...
1. Nella procedura guidata **Aggiungi infrastruttura di rete**, selezionare **Uscita porta flessibile**.
1. Dal menu a discesa **Area**, scegli l&#39;area desiderata, quindi fai clic su **Continua**.
...
Un nuovo record viene visualizzato sotto l&#39;intestazione **Infrastruttura di rete** nel pannello laterale. Include dettagli quali il tipo di infrastruttura, lo stato, l’area geografica e gli ambienti in cui è abilitata.
...
Se la funzione di indirizzo IP dedicato non è abilitata, il traffico da AEM as a Cloud Service passa attraverso un set condiviso di IP. Questi IP vengono utilizzati da altri clienti di AEM as a Cloud Service.

La configurazione di un indirizzo IP in uscita dedicato è simile all&#39;[uscita porta flessibile](#flexible-port-egress). La differenza principale è che, dopo la configurazione, il traffico sarà sempre in uscita da un IP dedicato e univoco. Per trovare tale IP, utilizza un risolutore DNS per identificare l’indirizzo IP associato a `p{PROGRAM_ID}.external.adobeaemcloud.com`. L’indirizzo IP non dovrebbe cambiare, ma se dovesse cambiare, sarà fornita una notifica avanzata.

>[!TIP]
>
>Dovendo decidere tra l’uscita flessibile della porta e l’indirizzo IP in uscita dedicato, conviene scegliere l’uscita flessibile della porta se non è necessario un indirizzo IP specifico. Il motivo è che Adobe può ottimizzare le prestazioni del traffico in uscita dalla porta flessibile.
...
1. Nella procedura guidata **Aggiungi infrastruttura di rete** visualizzata, fare clic su **Indirizzo IP in uscita dedicato**.
1. Dal menu a discesa **Area**, scegli l&#39;area desiderata, quindi fai clic su **Continua**.
...
1. Nella procedura guidata **Aggiungi infrastruttura di rete** che viene avviata, seleziona **Rete privata virtuale** e fornisci le informazioni necessarie prima di fare clic su **Continua**.
   ...
   * **Area geografica**: area in cui creare l&#39;infrastruttura.
   ...
   * **Chiave condivisa** - Chiave VPN già condivisa.
...
1. Nella finestra di dialogo **Aggiungi connessione**, definisci la tua connessione VPN, quindi fai clic su **Salva**.

   * **Nome connessione** - Nome descrittivo della connessione VPN, fornito nel passaggio precedente e che può essere aggiornato qui.
   * **Indirizzo** - Indirizzo IP del dispositivo VPN.
   * **Spazio indirizzi** - Intervalli di indirizzi IP da indirizzare tramite la VPN.
...
1. La finestra di dialogo si chiude e si torna alla scheda **Connessioni** della procedura guidata. Fai clic su **Continua**.
...
<tbody>
  ...
  <tr>
    <td></td>
    <td>Se l'IP non rientra nell'intervallo <i>Spazio degli indirizzi gateway VPN</i> e tramite la configurazione proxy http (configurata per impostazione predefinita per il traffico http/s utilizzando la libreria client Java™ HTTP standard)</td>
    <td>Qualsiasi</td>
    <td>Attraverso l’IP dedicato in uscita</td>
    <td></td>
  </tr>
...
<tbody>
  ...
  <tr>
    ...
    <td>N/D</td>
    <td>IP del gateway VPN sul lato AEM. Il team di progettazione della rete può utilizzare questo IP per consentire solo connessioni VPN al gateway VPN da un indirizzo IP specifico. </td>
  </tr>
</tbody>
</table>
...
* **Inoltro porte** - Le regole di inoltro porte devono essere dichiarate per tutte le porte di destinazione diverse da 80/443, ma solo se non si utilizza il protocollo http o https.
   ...
   * La connessione client che utilizza la porta 80/443 su http / https deve comunque utilizzare le impostazioni proxy nella connessione per applicare le proprietà della rete avanzata alla connessione.
...
* **Host non proxy**: gli host non proxy consentono di dichiarare un set di host che vanno instradati attraverso un intervallo di indirizzi IP condivisi anziché l’IP dedicato.
   * Questo approccio può essere utile in quanto il traffico in uscita attraverso IP condivisi può essere ulteriormente ottimizzato.
...
1. Fare clic su **Modifica** o **Elimina**.

1. Effettua una delle seguenti operazioni:

   * Se si sceglie **Modifica**, verrà aperta la procedura guidata **Modifica infrastruttura di rete**. Apporta le modifiche necessarie seguendo i passaggi descritti durante la creazione dell’infrastruttura.

   * Se hai scelto **Elimina**, conferma l&#39;eliminazione nella finestra di dialogo **Elimina configurazione di rete** con **Elimina** oppure interrompi con **Annulla**.
...
È possibile configurare un solo tipo di infrastruttura di rete avanzata per un programma alla volta. L’infrastruttura di rete avanzata deve essere in uscita da porta flessibile, indirizzo IP in uscita dedicato o VPN.
...
1. Se hai bloccato l’infrastruttura in modo che l’indirizzo IP AEM dedicato sia inserito nell’elenco Consentiti, dovresti disabilitare temporaneamente eventuali regole di rifiuto in tale infrastruttura. Se salti questo passaggio, l’infrastruttura rifiuta temporaneamente le richieste dagli indirizzi IP della nuova regione. Questo passaggio non è necessario se l&#39;infrastruttura è stata bloccata utilizzando un nome di dominio completo (FQDN), ad esempio `p1234.external.adobeaemcloud.com`. Tutte le aree geografiche di AEM ricevono il traffico di rete avanzato dallo stesso FQDN.
...
1. Se è necessario bloccare l&#39;infrastruttura tramite IP per consentire il traffico AEM, aggiungere gli IP corrispondenti a `p1234.external.adobeaemcloud.com`. Dovrebbe essercene uno per area geografica.
...
1. Crea un&#39;infrastruttura di rete per tutte le aree geografiche tramite una chiamata POST all&#39;[API Cloud Manager Create Network Infrastructure](https://developer.adobe.com/experience-cloud/cloud-manager/reference/api/#tag/Network-infrastructure/operation/createNetworkInfrastructure). L’unica differenza nella configurazione JSON del payload rispetto all’area primaria è la proprietà region.
...
La procedura è quasi identica alle istruzioni degli indirizzi IP in uscita dedicati. L’unica differenza consiste nel fatto che la proprietà region è configurata in modo diverso rispetto all’area primaria. Inoltre, è possibile configurare il campo `connections.gateway`. La configurazione può indirizzare a un endpoint VPN diverso gestito dalla tua organizzazione, geograficamente più vicino alla nuova area.
...
Il pool di connessioni è una tecnica ideata per creare e sostenere un archivio di connessioni, pronte per essere utilizzate fin da subito dai thread che possano richiederle. Su varie piattaforme e risorse online si possono trovare numerose tecniche di connection pooling, ciascuna con i suoi meriti e considerazioni particolari. Adobe incoraggia i clienti a studiare queste metodologie per identificare quella più compatibile con l’architettura del proprio sistema.

L’implementazione di un’adeguata strategia di connection pooling è una misura proattiva per correggere una svista comune nella configurazione del sistema, che spesso porta a prestazioni non ottimali. Stabilendo correttamente un connection pool, Adobe Experience Manager (AEM) può migliorare l’efficienza delle chiamate esterne. Questo approccio non solo riduce il consumo di risorse, ma riduce anche il rischio di interruzioni del servizio e diminuisce la probabilità di incontrare richieste non riuscite durante la comunicazione con i server a monte.

In base a queste informazioni, Adobe consiglia di rivedere la configurazione AEM corrente. È inoltre consigliabile utilizzare intenzionalmente il connection pooling insieme alle impostazioni di rete avanzate. La gestione del numero di connessioni parallele e la riduzione delle connessioni non aggiornate consentono di ottimizzare le prestazioni della rete. Queste azioni riducono il rischio che i server proxy raggiungano i limiti di connessione. Di conseguenza, questa implementazione strategica è progettata per ridurre la probabilità che le richieste non raggiungano gli endpoint esterni.

#### Domande frequenti sui limiti di connessione

Quando si utilizza la rete avanzata, il numero di connessioni è limitato per garantire la stabilità tra gli ambienti ed evitare che gli ambienti di dimensioni inferiori esauriscano le connessioni disponibili.
...
##### Non sembra esserci un aumento significativo del numero di connessioni in uscita. Perché la notifica viene ricevuta ora?
...
Quando viene raggiunto il limite massimo, le nuove connessioni in uscita da AEM tramite reti avanzate (in uscita su porte non standard, utilizzando IP in uscita dedicato o VPN) vengono eliminate per proteggere da un attacco DoS.
...
Sì, le connessioni vengono chiuse a livello di JVM e in punti diversi dell&#39;infrastruttura di rete. Tuttavia, questo flusso di lavoro è troppo tardi per qualsiasi servizio di produzione. Le connessioni devono essere chiuse in modo esplicito quando non sono più necessarie o restituite al pool quando si utilizza il pool di connessioni. In caso contrario, il consumo di risorse è troppo elevato e può causare l’esaurimento delle risorse.
...
Sì, il limite si applica alle connessioni che utilizzano reti avanzate, incluse le VPN.

##### Il limite si applica ancora quando si utilizza un IP in uscita dedicato?
