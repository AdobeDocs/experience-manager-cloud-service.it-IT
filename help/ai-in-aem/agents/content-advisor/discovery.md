---
title: Processo di individuazione contenuti
description: Scopri come utilizzare il processo di individuazione dei contenuti per fornire contenuti AEM pertinenti on-demand attraverso prompt conversazionali naturali per un’esperienza di individuazione semplice e senza clic.
...
# Processo di individuazione contenuti {#discovery-job}

Come parte dell&#39;[agente di Content Advisor](/help/ai-in-aem/agents/content-advisor/overview.md) di AEM, il processo di individuazione dei contenuti fornisce contenuti AEM on-demand attraverso messaggi di conversazione naturali per un&#39;esperienza di individuazione semplificata e senza clic. Esegue una ricerca intelligente tra Assets, Frammenti di contenuto e Forms adattivo per fornire materiali rilevanti come immagini, video, documenti PDF, articoli e modelli di modulo. Con il linguaggio naturale, puoi cercare contenuti senza creare query complesse o applicare filtri nell’interfaccia di AEM Assets. In base al prompt, il processo restituisce risultati curati insieme a metadati di risorse e URL di consegna, pronti per essere incorporati in altre applicazioni.

Alcuni dei vantaggi principali del processo di individuazione dei contenuti includono:
...
Il processo di individuazione dei contenuti fornisce le seguenti competenze:

* **Individuazione contenuti in linguaggio naturale**\
  Il processo di individuazione dei contenuti consente agli utenti di trovare risorse rilevanti, frammenti di contenuto e moduli adattivi all’interno di Adobe Experience Manager (AEM) utilizzando semplici prompt del linguaggio naturale, senza richiedere query di ricerca complesse.

* **Individuazione risorse basata su tag**

  Il processo di individuazione dei contenuti utilizza i prompt in linguaggio naturale per trovare le risorse associate a tag specifici nell’archivio AEM, consentendo agli utenti di accedere rapidamente ai contenuti organizzati o meno in base alla tassonomia dell’organizzazione.

* **Individuazione contenuto basato su cartelle:**\
  Il processo di individuazione dei contenuti può identificare le risorse interpretando i prompt del linguaggio naturale che fanno riferimento ai nomi delle cartelle in AEM. Gli utenti possono semplicemente menzionare la cartella nel prompt, senza navigare manualmente nell’archivio, riducendo in modo significativo il numero di clic necessari per individuare il contenuto corretto.

## Persone {#personas-content-discovery}

### Manager campagne {#campaign-managers}

Il processo di individuazione dei contenuti consente ai responsabili delle campagne di identificare e riutilizzare rapidamente i contenuti affidabili e ad alte prestazioni per la creazione.

### Responsabili marketing per canale {#channel-marketers}

Il processo di individuazione dei contenuti consente agli addetti al marketing di canale di trovare in modo efficiente le risorse rilevanti per creare esperienze coese e multicanale.
...
Puoi accedere al processo di individuazione dei contenuti in AEM tramite l’Assistente AI. Accedere a [`experience.adobe.com`](https://experience.adobe.com) e iniziare a interagire con l&#39;Assistente AI specificando il prompt nel linguaggio naturale utilizzando la casella di ricerca:

![Accedere al processo di individuazione contenuto](/help/ai-in-aem/agents/content-advisor/assets/access-discovery-agent.png)

Per informazioni sull’endpoint MCP per accedere al processo di individuazione dei contenuti, contatta il supporto Adobe.
...
Il processo di individuazione dei contenuti utilizza i prompt in linguaggio naturale per trovare le risorse associate a tag specifici nell’archivio AEM, consentendo agli utenti di accedere rapidamente ai contenuti organizzati in base alla tassonomia della propria organizzazione.
...
Il processo di individuazione dei contenuti è in grado di identificare le risorse che soddisfano specifici requisiti di qualità, ad esempio il formato di file, consentendo agli utenti di individuare rapidamente gli elementi visivi dei prodotti pronti per la distribuzione di alta qualità e il riutilizzo tra i canali.
...
## Risultati di ricerca {#discovery-job-search-results}

### Risorse {#discovery-job-search-results-assets}

Il processo di individuazione dei contenuti restituisce i primi risultati per ogni query, ordinati in base alla rilevanza per garantire che vengano visualizzate per prime le corrispondenze esatte. Il processo combina query guidate da metadati con ricerca semantica per assemblare un set mirato di corrispondenze probabili, quindi utilizza un LLM per classificarle in base alle intenzioni dell’utente. Questo approccio misto offre risultati precisi e in base al contesto, senza dipendere interamente da una corrispondenza diretta delle parole chiave.
...
![Cerca le risorse utilizzando il processo di individuazione contenuto](/help/ai-in-aem/agents/content-advisor/assets/search-results-discovery-agent.png)
...
Il processo di individuazione dei contenuti fornisce funzionalità di ricerca full-text per i frammenti di contenuto, restituendo i primi risultati che meglio corrispondono al prompt specificato. Ogni risultato include il nome del frammento di contenuto e i campi di metadati chiave, ad esempio il percorso del frammento di contenuto, il creatore, la data di creazione, le varianti, l’ultimo modificatore e i campi data dell’ultima modifica.

![Cerca frammenti di contenuto utilizzando il processo di individuazione contenuto](/help/ai-in-aem/agents/content-advisor/assets/search-content-fragments-discovery-agent.png)
...
## Best practice per la richiesta di assistenza {#prompting-best-practices-discovery-job}

Specifica dettagli concisi nei prompt della lingua naturale in modo che il processo possa restituire risultati precisi e pertinenti. Più si descrive chiaramente ciò che si sta cercando, meglio il lavoro può perfezionare e restringere l&#39;output. Ad esempio:
...
## Limitazioni {#limitations-discovery-job}

Il processo di individuazione dei contenuti supporta prompt basati sulle dimensioni solo per i tipi di formato immagine e SVG. Ad esempio, `Find images wider than 1080px`.
