...
1. In [!UICONTROL modificare i valori di configurazione] specificare il numero massimo di moduli o documenti per un&#39;istanza dell&#39;AEM [!DNL Forms Server] può memorizzare in cache **[!UICONTROL Numero di Forms adattivi]** campo. Il valore predefinito è 100.
...
L’ambiente AEM è configurato per memorizzare in cache il Forms adattivo. Memorizza nella cache tutti i tipi di Forms adattivo. Se devi controllare le autorizzazioni di accesso utente per una pagina prima di distribuire la pagina memorizzata in cache, consulta [caching di contenuto protetto](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/permissions-cache.html?lang=it).
...
Quando selezioni e aggiungi immagini o video tramite il browser di risorse a un modulo adattivo e questi vengono modificati nell’editor risorse, la validità di tali risorse non viene invalidata automaticamente dalla cache di Dispatcher.
...
>[!MORELIKETHIS]
>
>* [Risoluzione dei problemi relativi al caching per AEM Forms as a Cloud Service](/help/forms/troubleshooting-caching-performance.md)