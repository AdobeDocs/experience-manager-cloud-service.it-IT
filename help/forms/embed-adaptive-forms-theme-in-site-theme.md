...
Nel tema Forms, i file SCSS dei componenti spesso fanno riferimento a immagini con percorsi come `./resources/` o `url(resources/`. Dopo lo spostamento in `theme/src/components/adaptiveform/<component>/`, i percorsi devono puntare su un livello fino a `adaptiveform/resources/`.
...
Dopo la sostituzione, diventano rispettivamente `url(../resources/images/...)` e `url('../resources/images/...')`. Ripetere l&#39;operazione per ogni file SCSS in `adaptiveform/` che fa riferimento a immagini (pulsante, pannello a soffietto, procedura guidata, contenitore, scarabocchio e altri).
...
|-------|-------------------------------|
| Build non riuscita: &quot;file non trovato&quot; per un’immagine | Inserisci tutte le immagini del modulo in `theme/src/components/adaptiveform/resources/images/`. In ogni `.scss` in `adaptiveform/`, utilizzare `../resources/` (e `url(../resources/`) per i percorsi immagine, non `./resources/`. Se il bundler risolve i percorsi da `theme/src/`, inserisci le immagini in `theme/src/resources/images/` e utilizza `resources/images/` (nessun `../`) in SCSS. |
