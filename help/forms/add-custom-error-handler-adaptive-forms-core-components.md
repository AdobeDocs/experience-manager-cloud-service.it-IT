...
Aggiungiamo il seguente codice al file JavaScript per visualizzare la risposta e le intestazioni, ricevute dall’endpoint del servizio REST, nella console del browser.
   ...
   ```
   Nel codice riportato sopra, `return true` richiama automaticamente il gestore errori predefinito. Per evitare che il gestore degli errori predefinito venga chiamato per impostazione predefinita, includi `return false`.
